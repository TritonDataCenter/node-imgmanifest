...
var validators = {
    ...
    owner: function validateOwner(manifest) {
        var errs = [];
        if (! UUID_RE.test(manifest.owner)) {
    ...
    public: function validatePublic(manifest, options) {
        var errs = [];
        var public = boolFromString(manifest.public);
        if (typeof (public) !== 'boolean') {
            errs.push({
                field: 'public',
                code: 'Invalid'
            });
        } else if (options.mode === 'public' && !public) {
            errs.push({
                field: 'public',
                code: 'Invalid',
                message: 'private images are not allowed on a public Images API'
            });
        } else if (options.mode === 'private' && public) {
            errs.push({
                field: 'public',
                code: 'Invalid',
    ...
    type: function validateType(manifest) {
        ...
        if (VALID_TYPES[manifest.type] === undefined) {
            errs.push({
                field: 'type',
                code: 'Invalid',
    ...
    nic_driver: function validateNicDriver(manifest) {
        var errs = [];
        if (manifest.nic_driver && manifest.type !== 'zvol') {
            errs.push({
                field: 'nic_driver',
                code: 'Invalid',
                message: format(
    ...
    disk_driver: function validateDiskDriver(manifest) {
        var errs = [];
        if (manifest.disk_driver && manifest.type !== 'zvol') {
            errs.push({
                field: 'disk_driver',
                code: 'Invalid',
                message: format(
    ...
    cpu_type: function validateCpuType(manifest) {
        var errs = [];
        if (manifest.cpu_type && manifest.type !== 'zvol') {
            errs.push({
                field: 'cpu_type',
                code: 'Invalid',
                message: format(
    ...
    image_size: function validateImageSize(manifest) {
        var errs = [];
        if (manifest.image_size && manifest.type !== 'zvol') {
            errs.push({
                field: 'image_size',
                code: 'Invalid',
                message: format(
    ...
    os: function validateOs(manifest) {
        ...
        if (VALID_OSES[manifest.os] === undefined) {
            errs.push({
                field: 'os',
                code: 'Invalid',
    ...
    requirements: function validateRequirements(manifest) {
        var errs = [];
        var reqs = manifest.requirements;
        try {
    ...
    users: function validateUsers(manifest) {
        ...
        try {
            ...
            errs.push({
                ...
                code: 'Invalid',
                message: format(
                    'invalid image "users" (not parseable JSON): %s', users)
            });
        }
    ...
    billingtag: function validateBillingTags(manifest) {
        var errs = [];
        var bt = manifest.billingtag;
        if (typeof (bt) === 'string') {
            bt = [ bt ];
        } else if (!Array.isArray(bt)) {
            errs.push({
                field: 'billing_tags',
                code: 'Invalid',
    ...
    traits: function validateTraits(manifest) {
        ...
        try {
            ...
            errs.push({
                ...
                code: 'Invalid',
                message: format(
                    'invalid image "traits" (not parseable JSON): %s', traits)
            });
        }
    ...
    tag: function validateTags(manifest) {
        var errs = [];
        var tags = manifest.tag;
        if (typeof (tags) === 'string') {
            tags = keyValueToObject([tags]);
        } else {
            tags = keyValueToObject(tags);
        }
    ...
    inherited_directories: function validateInhDirectories(manifest) {
        ...
        try {
            ...
            errs.push({
                ...
                code: 'Invalid',
                message: format(
                'invalid image "inherited_directories" (not parseable JSON):' +
                ' %s', id)
            });
        }
    ...
    datacenter: function validateDatacenter(manifest) {
        var errs = [];
        if (typeof (manifest.datacenter) === 'string') {
            // pass through
        } else if (!Array.isArray(manifest.datacenter)) {
            errs.push({
                field: 'datacenters',
                code: 'Invalid',
                message: format('invalid image datacenters ' +
                    '(not an array): %s', manifest.datacenter)
            });
        }
        return errs;
    },
    ...
    origin: function validateOrigin(manifest) {
        var errs = [];
        if (! UUID_RE.test(manifest.origin)) {
...
function validateFields(manifest, requiredFields, options) {
    ...
    requiredFields.forEach(function (field) {
        if (manifest[field] === undefined) {
            errs.push({ field: field, code: 'MissingParameter' });
        } else {
