...
var validators = {
    ...
    destroyed_at: function validateDestroyedAt(manifest) {
        var errs = [];
        if (manifest.destroyed_at &&
            !PUBLISHED_AT_RE.test(manifest.destroyed_at)) {
            errs.push({
                field: 'destroyed_at',
                code: 'Invalid',
                message: 'destroyed_at date not in ' +
                         '"YYYY-MM-DDTHH:MM:SS(.SSS)Z" format'
            });
        }
        return errs;
    ...
    type: function validateType(manifest) {
        ...
        if (manifest.type === undefined) {
            ...
            // Allow type and os to have 'null' values when the image has not
            // been created yet
